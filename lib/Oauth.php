...
*/
abstract class Oauth extends Phirehose
{
    ...
    */
    ...
    /**
    */
    protected function prepareParameters($method = null, $url = null, array $params = null)
    {
        ...
        if (is_array($params)) {
            foreach ($params as $k => $v) {
                if (strncmp('@', $k, 1) !== 0) {
                    $sigParams[$k] = $this->encodeRfc3986($v);
                    $params[$k] = $this->encodeRfc3986($v);
                } else {
                    $params[substr($k, 1)] = $v;
                    unset($params[$k]);
                    $hasFile = true;
        ...
        // signing
        $oauth['oauth_signature'] = $this->encodeRfc3986($this->generateSignature($method, $url, $sigParams));
        return array('request' => $params, 'oauth' => $oauth);
    }

    protected function encodeRfc3986($string)
    {
    ...
    protected function generateSignature($method = null, $url = null, $params = null)
    {
        ...
        $concatenatedParams = $this->encodeRfc3986($concat);
        ...
        $normalizedUrl = $this->encodeRfc3986($retval);
        $method = $this->encodeRfc3986($method); // don't need this but why not?

        $signatureBaseString = "{$method}&{$normalizedUrl}&{$concatenatedParams}";

        # sign the signature string
        $key = $this->encodeRfc3986($this->consumerSecret?$this->consumerSecret:TWITTER_CONSUMER_SECRET) . '&' . $this->encodeRfc3986($this->password);
