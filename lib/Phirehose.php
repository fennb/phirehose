...
abstract class Phirehose
{
    ...
    * @todo Shouldn't really hard-code URL strings in this function.
    ...
    {
        $this->log(
            'Consume rate: ' . $this->statusRate .
            ' status/sec (' . $this->statusCount . ' total), avg ' .
            'enqueueStatus(): ' . $this->enqueueTimeMS . 'ms, avg checkFilterPredicates(): ' .
            $this->filterCheckTimeMS . 'ms (' . $this->filterCheckCount . ' total) over ' .
            $this->avgElapsed . ' seconds, max stream idle period: ' .
        ...
        do {
            ...
            if (!$this->conn || !is_resource($this->conn)) {
                ...
                if ($connectFailures > $this->connectFailuresMax) {
                    ...
                    throw new Exception\ConnectLimitExceeded($msg, $errNo); // Throw an exception for other code to handle
                }
            ...
            if ($httpCode != 200) {
                ...
                if ($connectFailures > $this->connectFailuresMax) {
                    ...
                    throw new Exception\ConnectLimitExceeded($msg, $httpCode); // We eventually throw an exception for other code to handle
                }
                // Increase retry/backoff up to max
                ...
                if (!$isChunking) {
                    throw new Exception\PhirehoseException("Twitter did not send a chunking header. Is this really HTTP/1.1? Here are headers:\n$respHeaders");   //TODO: rather crude!
                }
            }
    ...
    {
        throw new Exception\PhirehoseException("Basic auth no longer works with Twitter. You must derive from OauthPhirehose, not directly from the Phirehose class.");
