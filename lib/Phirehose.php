...
abstract class Phirehose
{
    ...
    public function setLocationsByCircle($locations)
    {
        ...
        foreach ($locations as $locTriplet) {
            ...
            if (count($locTriplet) != 3) {
                ...
                $this->log(
                    'Invalid location triplet for ' . __METHOD__ . ': [' . implode(', ', $locTriplet) . ']',
                    'error'
                );
            ...
            $boundingBoxes[] = [$minLon, $minLat, $maxLon, $maxLat];
            // Debugging is handy
            $this->log(
                'Resolved location circle [' . $lon . ', ' . $lat . ', r: ' . $radius . '] -> bbox: [' . $minLon .
    ...
    {
        @trigger_error('Phirehose: ' . $message, $this->errorTypes[$level]);
    }
    ...
    /**
     * Returns the ISO 639-1 code formatted language string of the current setting.
     *
    ...
    public function consume($reconnect = true)
    {
        ...
        do {
            ...
            while ($this->conn !== null && !feof($this->conn) &&
               ...
               */
                if ((time() - $lastStreamActivity) > $this->idleReconnectTimeout) {
                    $this->log(
                        'Idle timeout: No stream activity for > ' . $this->idleReconnectTimeout . ' seconds. ' .
                        ' Reconnecting.',
                        'info'
                    );
                    $this->reconnect();
                ...
                if ($this->avgElapsed >= $this->avgPeriod) {
                    $this->statusRate = round(
                        $this->statusCount / $this->avgElapsed,
                        0
            ...
            $this->lastErrorMsg = ($this->lastErrorNo > 0) ? @socket_strerror(
                $this->lastErrorNo
            ) : 'Socket disconnected';
    ...
    protected function connect()
    {
        ...
        do {
            ...
            if (($this->method == self::METHOD_FILTER || $this->method == self::METHOD_USER) && count(
                    $this->trackWords
                ) > 0
            ) {
            ...
            // Debugging is useful
            $this->log(
                'Connecting to twitter stream: ' . $url . ' with params: ' . str_replace(
                    "\n",
                    '',
            ...
            $this->log(
                "Connecting to {$scheme}{$urlParts['host']}, port={$port}, connectTimeout={$this->connectTimeout}"
            );
            ...
            if (!$this->conn || !is_resource($this->conn)) {
                ...
                if ($connectFailures > $this->connectFailuresMax) {
                    ...
                    $this->log($msg, 'error');
                    throw new PhirehoseConnectLimitExceeded(
                        $msg, $errNo
                ...
                $this->log(
                    'TCP failure ' . $connectFailures . ' of ' . $this->connectFailuresMax . ' connecting to stream: ' .
            ...
            if ($httpCode != 200) {
                ...
                $this->log(
                    'HTTP failure ' . $connectFailures . ' of ' . $this->connectFailuresMax . ' connecting to stream: ' .
                ...
                if (!$isChunking) {
                    throw new Exception(
                        "Twitter did not send a chunking header. Is this really HTTP/1.1? Here are headers:\n$respHeaders"
                    );
                }   //TODO: rather crude!
            }
    ...
    {
        throw new Exception(
            "Basic auth no longer works with Twitter. You must derive from OauthPhirehose, not directly from the Phirehose class."
        );
    ...
    {
        $this->log(
            'Consume rate: ' . $this->statusRate . ' status/sec (' . $this->statusCount . ' total), avg ' .
